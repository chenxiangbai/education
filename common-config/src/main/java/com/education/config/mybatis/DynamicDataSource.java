...
/**
 * @Author admin
 * @Date 2021-02-23 14:39
 * @Version 1.0
 * @Description 数据源切换创建等操作
 */
...
//                    throw new ADIException("不存在的数据源："+datasource,500);
    ...
    // 创建数据源
    public boolean createDataSource(String key, String driveClass, String url, String username, String password, String dbType) {
        try {
            try { // 排除连接不上的错误
...
//            }
    ...
    private  void createDataSource(DataSource dataSource) throws Exception {
        ...
        String driveClass = dataSource.getDriveClass();;
//        if("mysql".equalsIgnoreCase(databasetype)) {
//            driveClass = DBUtil.mysqldriver;
//        } else if("oracle".equalsIgnoreCase(databasetype)){
//            driveClass = DBUtil.oracledriver;
...
//        }
        if(testDatasource(driveClass,url,username,password)) {
            boolean result = this.createDataSource(datasourceId, driveClass, url, username, password, dbType);
            if(!result) {
